...
extension ComposerPrintDraftExtension on ComposerController {
  ...
  Future<void> printDraft(BuildContext context) async {
    ...
    if (resultState is PrintEmailFailure) {
      ...
      if (context.mounted) {
        appToast.showToastErrorMessage(context, appLocalizations.printDraftFailed);
      }
    ...
    final childWidget = PointerInterceptor(
      child: PrintDraftDialogView(
        emailPrint: EmailPrint(
          ...
          toAddress: listToEmailAddress.toSet().listEmailAddressToString(isFullEmailAddress: true),
          ccAddress: listCcEmailAddress.toSet().listEmailAddressToString(isFullEmailAddress: true),
          bccAddress: listBccEmailAddress.toSet().listEmailAddressToString(isFullEmailAddress: true),
